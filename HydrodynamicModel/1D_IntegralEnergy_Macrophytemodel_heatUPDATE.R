...
source('1D_macrophyte-functions_heatUPDATE.R')
...
# macrophyte 
macro_all <- get_macrophyte(canopyfile = 'bc/canopy_F19_simNONE.csv',
                            biomassfile = 'bc/biomass_F19_simNONE.csv',
                            processed_meteo = meteo_all[[1]])
...
# having issues with rmse calculation - obs df is longer than m.df.sim.interp
# trying to calculate manually
# join observation and simulation dataframes
rmse_manual_calc<-merge(x=m.df.sim.interp, y=m.obs, by=c("datetime","variable"))
rmse_manual_calc$diff<-(rmse_manual_calc$value.y-rmse_manual_calc$value.x) #observed temp - modeled temp
rmse_manual_calc$diff_sq<-rmse_manual_calc$diff*rmse_manual_calc$diff

rmse_0m<-subset(rmse_manual_calc, variable=="temp_c.0")
sqrt(mean(rmse_0m$diff_sq)) #RMSE = 2.47C

rmse_0.25m<-subset(rmse_manual_calc, variable=="temp_c.0.25")
sqrt(mean(rmse_0.25m$diff_sq)) #RMSE = 1.91C

rmse_0.5m<-subset(rmse_manual_calc, variable=="temp_c.0.5")
sqrt(mean(rmse_0.5m$diff_sq, na.rm=T)) #RMSE = 1.61C

rmse_0.75m<-subset(rmse_manual_calc, variable=="temp_c.0.75")
sqrt(mean(rmse_0.75m$diff_sq)) #RMSE = 1.64C

rmse_1m<-subset(rmse_manual_calc, variable=="temp_c.1")
sqrt(mean(rmse_1m$diff_sq)) #RMSE = 1.79C

rmse_1.25m<-subset(rmse_manual_calc, variable=="temp_c.1.25")
sqrt(mean(rmse_1.25m$diff_sq)) #RMSE = 1.34C

rmse_1.5m<-subset(rmse_manual_calc, variable=="temp_c.1.5")
sqrt(mean(rmse_1.5m$diff_sq)) #RMSE = 1.05C

rmse_2m<-subset(rmse_manual_calc, variable=="temp_c.2")
sqrt(mean(rmse_2m$diff_sq)) #RMSE = 1.52C
...
m.obs.fig<-m.obs %>% 
  ...
  mutate(variable=(recode(variable, "temp_c.0" = "0 m", "temp_c.0.25"="0.25 m", "temp_c.0.5"="0.5 m",
                "temp_c.0.75"="0.75 m", "temp_c.1"="1 m","temp_c.1.25"="1.25 m", "temp_c.1.5"= "1.5 m", "temp_c.2"="2 m")))
...
write.csv(m.df.sim.interp.fig, "model_out_F19_NEW.csv",row.names = F)
write.csv(m.df.sim.interp.fig, "model_out_NOMAC_NEW.csv",row.names = F)
...
modelgoodnessplot<-
  ...
  theme_linedraw()  + theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank())
ggsave("Model_Fit_F19.png",modelgoodnessplot, width = 7, height = 11, units = 'in')
ggsave("Model_Fit_B19.png",modelgoodnessplot, width = 7, height = 11, units = 'in')
